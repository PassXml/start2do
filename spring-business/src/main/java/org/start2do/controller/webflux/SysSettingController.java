...
public class SysSettingController {
  ...
  public Mono<R<Page<SettingPageResp>>> page(SettingPageReq req) {
    ...
    Where.ready()
        .like(req.getKey(), qClass.key::like)
        .notEmpty(req.getType(), qClass.type::eq)
        .like(req.getValue(), qClass.value::like);
    return Mono.just(settingService.page(qClass, req, SettingDtoMapper.INSTANCE::toSettingResp))
        .map(R::ok);
  }
  ...
  public Mono<R<Boolean>> update(@RequestBody SettingUpdateReq req) {
    ...
    return Mono.fromCallable(
            () -> {
              ...
              if (!setting.getType().equals(req.getType())) {
                throw new BusinessException("内置参数不允许修改Type");
              }
  ...
  public Mono<R<Integer>> delete(IdReq req) {
    ...
    if (setting != null && setting.getIsBuiltIn() == YesOrNoType.Yes) {
      return Mono.error(new BusinessException("内置参数不允许删除"));
    }
  ...
  public Mono<R<SettingDetailResp>> detail(IdReq req) {
    BeanValidatorUtil.validate(req);
    return Mono.fromCallable(
            () -> SettingDtoMapper.INSTANCE.toDetail(settingService.getById(req.getId())))
        .map(R::ok);
  }
}
