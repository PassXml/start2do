...
public class PermissionServiceImpl extends AbsService<SysPermission>
    implements IPermissionService, CommandLineRunner {

  @Lazy @Resource private AbsPermissionController permissionController;

  public void reloadAllUrls() {
    Set<PermissionDto> urls = permissionController.getAllUrls();
    Set<String> set =
        new QSysPermission()
            .findList().stream().map(SysPermission::getUrl).collect(Collectors.toSet());
    DiffDTO<PermissionDto, String> dto =
        ListUtil.diff(urls, set, (permissionDto, s) -> permissionDto.getUrls().contains(s));
    for (PermissionDto t : dto.getAddList()) {
      for (String url : t.getUrls()) {
        ...
        new SysPermissionRoleRef(
                new SysPermissionRoleRefId(
                    md5,
                    SysSettingUtil.getLabel(
                        Constant.TYPE_SYSTEM_SETTING, Constant.KEY_ADMIN_ROLE, "1")))
            .save();
      }
    }
  ...
  public boolean assignToUsers(String permissionId, List<String> userIds) {
    ...
    List<SysPermissionUserRef> list =
        new QSysPermissionUserRef().permissionId.eq(permissionId).findList();
    DiffDTO<String, SysPermissionUserRef> dto =
        ListUtil.diff(
            userIds, list, (s, sysPermissionRoleRef) -> sysPermissionRoleRef.getUserId().equals(s));
    ...
    ListUtil.splitAfterRun(
        ...
        spList -> {
          new QSysPermissionUserRef()
              ...
              .eq(permissionId)
              .userId
              .in(spList.stream().map(SysPermissionUserRef::getUserId).collect(Collectors.toList()))
              .delete();
        });
    return true;
  }
  ...
  public boolean assignToRoles(String permissionId, List<String> roleIds) {
    ...
    List<SysPermissionRoleRef> list =
        new QSysPermissionRoleRef().permissionId.eq(permissionId).findList();
    DiffDTO<String, SysPermissionRoleRef> dto =
        ListUtil.diff(
            roleIds, list, (s, sysPermissionRoleRef) -> sysPermissionRoleRef.getRoleId().equals(s));
    for (String s : dto.getAddList()) {
    ...
    ListUtil.splitAfterRun(
        ...
        spList -> {
          new QSysPermissionRoleRef()
              ...
              .eq(permissionId)
              .roleId
              .in(spList.stream().map(SysPermissionRoleRef::getRoleId).collect(Collectors.toList()))
              .delete();
        });
    return true;
  }
  ...
  public Page<SysPermission> page(PermissionPageReq request) {
    ...
    // 执行查询
    PagedList<SysPermission> pagedList =
        query.setFirstRow(request.getOffset()).setMaxRows(request.getSize()).findPagedList();

    return new Page<>(
        pagedList.getTotalCount(), request.getSize(), request.getCurrent(), pagedList.getList());
  }
  ...
  public PermissionDetailResp getDetail(String id, boolean isUser) {
    ...
    // 构建详情DTO
    PermissionDetailResp detailDto =
        new PermissionDetailResp()
    ...
    // 转换用户列表
    if (isUser) {
      List<PermissionDetailResp.UserDto> userDtos =
          permission.getUsers().stream()
              .map(
                  user ->
                      new PermissionDetailResp.UserDto()
                          .setId(user.getId())
    ...
    // 转换角色列表
    if (!isUser) {
      List<PermissionDetailResp.RoleDto> roleDtos =
          permission.getRoles().stream()
              .map(
                  role ->
                      new PermissionDetailResp.RoleDto()
                          .setId(role.getId())
