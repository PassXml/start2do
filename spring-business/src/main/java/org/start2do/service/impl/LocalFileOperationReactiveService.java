...
public class LocalFileOperationReactiveService implements IFileOperationService {
  ...
  public Mono<SysFile> uploadReactive(byte[] bytes, String fileName, Boolean checkExist) {
    ...
    return Mono.fromCallable(
            () -> {
              ...
              if (checkExist) {
                return sysFileReactiveService
                    .findOneReactive(new QSysFile().fileMd5.eq(md5))
                    .switchIfEmpty(
  ...
  public Mono<Boolean> downloadReactive(ServerHttpResponse response, String fileId) {
    ...
    return sysFileReactiveService
        .findOneByIdReactive(fileId)
        .flatMap(
            sysFile -> {
              response
                  .getHeaders()
                  .add("Content-Disposition", "attachment;filename=" + sysFile.getFileName());
              ...
              try (FileInputStream inputStream = new FileInputStream(file)) {
                Flux<DataBuffer> dataBufferFlux =
                    DataBufferUtils.readByteChannel(
                        inputStream::getChannel, new DefaultDataBufferFactory(), 4096);
