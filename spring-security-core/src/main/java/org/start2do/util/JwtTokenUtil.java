...
public class JwtTokenUtil implements Serializable {
    ...
    public String generateToken(UserCredentials userCredentials) {
        return doGenerateToken(userCredentials, null);
    }

    public String generateToken(UserCredentials userCredentials, Map<String, Object> extMap) {
        return doGenerateToken(userCredentials, extMap);
    }

    private String doGenerateToken(UserCredentials userCredentials, Map<String, Object> extMap) {
        try {
            // 使用 nimbus-jose-jwt 进行 JWE 加密
            Builder claim = new Builder().subject(String.valueOf(userCredentials.getId()))
                .issueTime(new Date(System.currentTimeMillis()))
                .expirationTime(new Date(System.currentTimeMillis() + JWT_TOKEN_VALIDITY * 1000))
                .claim(USERNAME, userCredentials.getUsername()).claim(MENUS, userCredentials.getMenus())
                .claim(ROLES, userCredentials.getRoles()).claim(REALNAME, userCredentials.getRealName());
            if (extMap != null) {
                extMap.forEach(claim::claim);
            }
            com.nimbusds.jwt.JWTClaimsSet claimsSet = claim.build();

            com.nimbusds.jwt.SignedJWT signedJWT = new com.nimbusds.jwt.SignedJWT(
                new com.nimbusds.jose.JWSHeader(JWSAlgorithm.HS256), claimsSet);
            ...
            com.nimbusds.jose.JWEObject jweObject = new com.nimbusds.jose.JWEObject(
                new com.nimbusds.jose.JWEHeader.Builder(com.nimbusds.jose.JWEAlgorithm.DIR,
                    com.nimbusds.jose.EncryptionMethod.A256GCM).contentType("JWT").build(),
