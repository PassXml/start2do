...
public class ScriptRunner {
    ...
    public static ScriptRunnerResult evalNoCache(Type implKey, String script, Object... objects) {
        IScriptRunner runner = map.get(implKey);
        if (runner == null) {
            throw new RuntimeException("没有配置脚本执行器");
        }
        return runner.evalNoCache(script, objects);
    }

    public static ScriptRunnerResult eval(String script, Object... objects) {
        return eval(defaultImpl, script, objects);
    }

    public static ScriptRunnerResult eval(Type implKey, String scprit, Object... objects) {
        IScriptRunner runner = map.get(implKey);
        if (runner == null) {
            throw new RuntimeException("没有配置脚本执行器");
        }
